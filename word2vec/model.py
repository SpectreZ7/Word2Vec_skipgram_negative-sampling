"""
Word2Vec skip-gram model: embeddings, forward pass, loss, and gradients.

TWO EMBEDDING MATRICES
----------------------
Word2Vec maintains two separate matrices:

  W_in  (V x d): "input" embeddings, one row per word. Used when a word
                  appears as the CENTER of a training window.

  W_out (V x d): "output" embeddings, one row per word. Used when a word
                  appears as a CONTEXT word (positive or negative sample).

This asymmetry is intentional. The two roles are different: W_in encodes
"what this word means", W_out encodes "what words tend to appear near this word".
After training we use W_in (or W_in + W_out averaged) as the final embeddings.

SKIP-GRAM WITH NEGATIVE SAMPLING (SGNS)
-----------------------------------------
Objective (for ONE training pair (c, o) with K negative samples n_1..n_K):

    L = log σ( v_o · u_c )  +  Σ_{k=1}^{K} log σ( −v_{nk} · u_c )

where:
    u_c   = W_in[c]       center-word embedding    (d,)
    v_o   = W_out[o]      true-context embedding   (d,)
    v_{nk}= W_out[n_k]    noise-word embedding     (d,)
    σ(x)  = 1/(1+e^{-x}) sigmoid function

Interpretation:
  - The first term pushes the dot product of the true context and center word UP.
  - Each negative term pushes the dot product of a noise word and the center DOWN.
  - We train a binary classifier: is this (center, context) pair real or noise?

We MINIMISE the negative of L:

    loss = −log σ(s_pos)  −  Σ_k log σ(−s_{nk})
...
Let s_pos = v_o · u_c  and  s_{nk} = v_{nk} · u_c.

Gradient of L (to maximise) w.r.t. each parameter:

    ∂L/∂u_c    = (1 − σ(s_pos)) · v_o  −  Σ_k σ(s_{nk}) · v_{nk}
    ∂L/∂v_o    = (1 − σ(s_pos)) · u_c
    ∂L/∂v_{nk} = −σ(s_{nk}) · u_c

SGD update (ascending L, i.e. descending the loss):
    param += lr * ∂L/∂param

BATCHED VARIANT
---------------
backward_and_update_batch processes B pairs simultaneously.
The math is identical — shapes just gain a leading batch dimension B.
This is required for GPU efficiency: launching a kernel per pair is slower
than CPU; batching amortises the launch overhead across B pairs.
"""
